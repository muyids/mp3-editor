...
import com.mpatric.mp3agic.ID3Genres;
...
public class Mp3Retag extends BaseApp {
    ...
    protected static String artist = null;

    protected static String album = null;

    protected static String year = null;

    protected static String genre = null;

    protected static String albumArtist = null;
    ...
    protected Mp3Retag(String filename) {
        try {
            ...
            if (hasId3v2Tag && mp3file.getId3v2Tag().getAlbumImage() != null) {
                hasImage = true;
            }
            if (!hasId3v1Tag && !hasId3v2Tag) {
                ...
                message.append("\"");
                if (!hasId3v1Tag) {
                    message.append(", added ID3v1 tag");
                }
                if (!hasId3v2Tag) {
                    message.append(", added ID3v2 tag");
                }
                ...
                if (!hasCustomTag) {
                    if (mp3file.hasCustomTag()) {
                        message.append(", added custom tag");
                    }
                } else {
                    if (!mp3file.hasCustomTag()) {
                        message.append(", removed custom tag");
                    } else if (keepCustomTag && customTag != null && customTag.length() > 0) {
                        message.append(", appended to custom tag");
                    } else {
                        message.append(", replaced custom tag");
                    }
                }
            ...
            if (mp3file != null) {
                deleteFile(mp3file.getFilename() + RETAG_EXTENSION);
            }
        } catch (Exception e) {
    ...
    private void updateId3Tags() {
        ...
        if (year != null) {
            newId3Wrapper.setYear(trimField(year));
        } else {
            newId3Wrapper.setYear(trimField(oldId3Wrapper.getYear()));
        }
        if (genre != null) {
            newId3Wrapper.setGenre(ID3Genres.matchGenreDescription(genre));
        } else {
        ...
        if (title != null) {
            newId3Wrapper.setTitle(trimField(title));
        } else {
            newId3Wrapper.setTitle(trimField(oldId3Wrapper.getTitle()));
        }
        if (artist != null) {
            newId3Wrapper.setArtist(trimField(artist));
        } else {
            newId3Wrapper.setArtist(trimField(oldId3Wrapper.getArtist()));
        }
        if (album != null) {
            newId3Wrapper.setAlbum(trimField(album));
        } else {
            newId3Wrapper.setAlbum(trimField(oldId3Wrapper.getAlbum()));
        }
    ...
    private boolean findAndSetAlbumImage(ID3Wrapper id3Wrapper) {
        ...
        System.out.println(id3Wrapper.getArtist());
        System.out.println(id3Wrapper.getAlbum());
    ...
    protected static boolean parseArgs(String args[]) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].charAt(0) == '-') {
                ...
                } else if ("--albumArtist".equals(args[i])) {
                    try {
                        albumArtist = args[i + 1];
                        i++;
                    } catch (ArrayIndexOutOfBoundsException e) {
                        return false;
                    }
